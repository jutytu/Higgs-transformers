...
def input_vectors(df): 
    ...
    features_gen1 = ['eta_gentau_1', 'phi_gentau_1', 'pt_gentau_1']
    features_gen2 = ['eta_gentau_2', 'phi_gentau_2', 'pt_gentau_2']
    ...
    df1['cos_ver_a1'] = compute_angle(a11, ver1)
    df2['cos_ver_a1'] = compute_angle(a12, ver2)
    
    df1['arccos'] = np.arcsin((tau1.E**2-tau1.px**2-tau1.py**2-tau1.pz**2-(a11.E**2-a11.px**2-a11.py**2-a11.pz**2))/(2*np.sqrt(tau1.E**2-tau1.px**2-tau1.py**2-tau1.pz**2)*np.sqrt(a11.px**2+a11.py**2+a11.pz**2)))
    df2['arccos'] = np.arcsin((tau2.E**2-tau2.px**2-tau2.py**2-tau2.pz**2-(a12.E**2-a12.px**2-a12.py**2-a12.pz**2))/(2*np.sqrt(tau2.E**2-tau2.px**2-tau2.py**2-tau2.pz**2)*np.sqrt(a12.px**2+a12.py**2+a12.pz**2)))
    ...
    # df_gen1['delta_R'] = np.sqrt((df['eta_tau_1']-df['eta_nu1'])**2+(df['phi_tau_1']-df['phi_nu1'])**2)
    # df_gen2['delta_R'] = np.sqrt((df['eta_tau_2']-df['eta_nu2'])**2+(df['phi_tau_2']-df['phi_nu2'])**2)
    y_gen1 = torch.tensor(df_gen1.values, dtype=torch.float32)
    y_gen2 = torch.tensor(df_gen2.values, dtype=torch.float32)
    
    weights = torch.stack([torch.tensor(df['spinner_wt_cp_even'].values, dtype=torch.float32), torch.tensor(df['spinner_wt_cp_odd'].values, dtype=torch.float32), torch.tensor(df['spinner_wt_cp_mm'].values, dtype=torch.float32)], dim=1)   
    
    # y_target1 = torch.tensor(df['phicp_gen'].values, dtype=torch.float32)
    # y_target2 = torch.tensor(df['phicp_gen'].values*weights[:, 0].detach().numpy(), dtype=torch.float32)
    ...
    y_target = torch.stack((y_gen1, y_gen2), dim=1)
    y_target = y_target.view(y_target.shape[0], 6)
    print(y_target.shape)
    print(tau_matrix.shape)
    # y_target = torch.unsqueeze(y_target, dim=-1)
    
    

    return tau_matrix, other_matrix, decay_matrix, y_target, weights
...
class TauTransformer(nn.Module):
    def __init__(self, embed_dim=128, num_heads=2, ff_hidden_dim=64, dropout=0.1):
        ...
        self.embedding_tau = EmbeddingTau(input_dim=9, output_dim=embed_dim)          # here for decay info
...
tau_matrix, other_matrix, decay_matrix, y_target, weights = input_vectors(df_train)
angles_valid = eta_phi_target(df_valid)
tau_matrix_v, other_matrix_v, decay_matrix_v, y_target_v, weights_v = input_vectors(df_valid)
...
#     print(f"Epoch {epoch}: train loss {average_train_loss:.4f}, val loss {average_val_loss:.4f}, lr {current_lr:.6f}")

torch.save(model, 'may2.pth')


## test is actually the whole df!!

tau_matrix, other_matrix, decay_matrix, y_target, weights = input_vectors(df)
model.eval()
test_preds = model(tau_matrix, other_matrix, decay_matrix)

# target_phi = np.arctan2(y_target[:, 0].detach().numpy(), y_target[:, 1].detach().numpy()) 
# test_phi = np.arctan2(test_preds[:, 0].detach().numpy(), test_preds[:, 1].detach().numpy()) 
# target_phi = y_target[:, 0].detach().numpy()
# test_phi = test_preds[:, 0].detach().numpy()

# plt.figure()
# plt.hist(target_phi, bins=100, alpha=0.5, label='gen')
# plt.hist(test_phi, bins=100, alpha=0.5, label='regr')
# plt.legend()
# plt.title('phi CP distribution')

# plt.figure()
# plt.hist(y_target[:, 1].detach().numpy(), bins=100, alpha=0.5, label='gen')
# plt.hist(test_preds[:, 1].detach().numpy(), bins=100, alpha=0.5, label='regr')
# plt.legend()
# plt.title('phi CP distribution - even')


# plt.figure()
# plt.hist(target_phi, bins=100, alpha=0.5, label='gen', weights=weights[:, 0].detach().numpy())
# plt.hist(test_phi, bins=100, alpha=0.5, label='regr', weights=weights[:, 0].detach().numpy())
# plt.legend()
# plt.title('phi CP distribution - CP even weights')

# plt.figure()
# plt.hist(target_phi, bins=100, alpha=0.5, label='gen', weights=weights[:, 1].detach().numpy())
# plt.hist(test_phi, bins=100, alpha=0.5, label='regr', weights=weights[:, 1].detach().numpy())
# plt.legend()
# plt.title('phi CP distribution - CP odd weights')
...
# plt.title('test preds - pz1')
# plt.show()

# plt.figure()
# plt.hist(y_target[:, 3].detach().numpy(), bins=100, alpha=0.5)
# plt.hist(test_preds[:, 3].detach().numpy(), bins=100, alpha=0.5)
# plt.title('test preds - delR 1')
# plt.show()

# plt.figure()
# plt.hist(y_target[:, 7].detach().numpy(), bins=100, alpha=0.5)
# plt.hist(test_preds[:, 7].detach().numpy(), bins=100, alpha=0.5)
# plt.title('test preds - delR 2')
# plt.show()
...
plt.hist(df['phi_tau_1'].values, bins=100, alpha=0.3, label='reconstructed', color='red')
plt.title('reconstructed- phi1')
plt.ylabel('counts')
plt.xlabel('phi1')
plt.legend()
plt.show()

print(len(y_target[:, 0].detach().numpy()))
print(len(test_preds[:, 0].detach().numpy()))

plt.figure()
plt.hist(y_target[:, 0].detach().numpy(), bins=100, alpha=0.5, label='generated')
plt.hist(df['eta_tau_1'].values, bins=100, alpha=0.3, label='reconstructed', color='red')
plt.title('reconstructed - eta1')
plt.xlabel('eta1')
plt.ylabel('counts')
plt.legend()
plt.show()
...
# test_preds_np = test_preds.detach().numpy()
# px1, py1, pz1 = test_preds_np[:, 0], test_preds_np[:, 1], test_preds_np[:, 2]
# px2, py2, pz2 = test_preds_np[:, 3], test_preds_np[:, 4], test_preds_np[:, 5]
# angles_test = eta_phi_target(df).detach().numpy()

# eta1 = 0.5 * np.log((np.sqrt(px1**2 + py1**2 + pz1**2) + pz1) / (np.sqrt(px1**2 + py1**2 + pz1**2) - pz1))
# eta2 = 0.5 * np.log((np.sqrt(px2**2 + py2**2 + pz2**2) + pz2) / (np.sqrt(px2**2 + py2**2 + pz2**2) - pz2))
# phi1 = np.arctan2(py1, px1)
# phi2 = np.arctan2(py2, px2)

# print(len(eta1))
# print(len(angles_test[:, 0]))


# plt.figure()
# plt.hist(angles_test[:, 0], bins=100, alpha=0.5)
# plt.hist(eta1, bins=100, alpha=0.5)
# plt.title('eta1 regr')
# plt.show()

# plt.figure()
# plt.hist(angles_test[:, 1], bins=100, alpha=0.5)
# plt.hist(phi1, bins=100, alpha=0.5)
# plt.title('phi1 regr')
# plt.show()
